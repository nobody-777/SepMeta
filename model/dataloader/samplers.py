...
class CategoriesSampler_SPL():

    def __init__(self, label, n_batch, n_cls, n_per, classes):
        self.n_batch = n_batch
        self.n_cls = n_cls
        self.n_per = n_per

        self.classes = classes
        self.n_batch_chosen = len(classes)
    ...
    def __len__(self):
        return self.n_batch_chosen

    def __iter__(self):
        for i_batch in range(self.n_batch_chosen):
            # batch = []
            # # classes = torch.randperm(len(self.m_ind))[:self.n_cls]
            # classes = self.classes[i_batch]
            # for c in classes:
            #     l = self.m_ind[c]
            #     pos = torch.randperm(len(l))[:self.n_per]
            #     batch.append(l[pos])
            ...
            for _ in [0, 1]:
                ...
                for c in classes:
                    ...
                    batch.append(l[pos])
                batch = torch.stack(batch).t().reshape(-1)
                pseudo_batches.append(batch)

            yield torch.cat(pseudo_batches)
