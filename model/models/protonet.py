...
class ProtoNet(FewShotModel):
    ...
    def _forward(self, instance_embs, support_idx, query_idx):
        ...
        proto_q = query.mean(dim=1) # Ntask x Nq x d
        ...
        # if self.training:
        #     # print('# construct hard task ...')
        #     for i in range(num_proto):
        #         other_classes_in_q = proto_q[0][torch.arange(proto_q[0].size(0))!=i]
        #         lambda_ = torch.FloatTensor(1).uniform_(0, 0.3).cuda()
        #         proto[0][i] = lambda_ * torch.mean(other_classes_in_q,0) + (1-lambda_) * proto[0][i]
